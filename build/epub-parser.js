...
export class EpubParser {
     ...
     * 提取元数据
     */
    async extractMetadata(opfContent) {
        const metadata = opfContent?.package?.metadata?.[0];
        if (!metadata) {
            return {};
        }
        const getFirst = (field) => field?.[0] || '';
        return {
     ...
     * 提取章节内容
     */
    async extractChapters(opfContent) {
        const spine = opfContent?.package?.spine?.[0]?.itemref;
        const manifest = opfContent?.package?.manifest?.[0]?.item;
        if (!spine || !manifest) {
            return [];
     ...
     * 提取章节内容
     */
    async extractChapterContent(href) {
     ...
     * 搜索内容
     */
    async searchContent(query, caseSensitive = false) {
        const info = await this.getInfo();
        const results = [];
        for (const chapter of info.chapters) {
            const content = chapter.content;
            const regex = new RegExp(caseSensitive ? query : query.toLowerCase(), caseSensitive ? 'g' : 'gi');
            let match;
