...
class EpubParserServer {
      ...
      async (request) => {
        ...
        try {
            ...
            case 'chapter':
              if (!chapterId) {
                throw new McpError(
                  ...
                  'Chapter ID is required for chapter resource'
                );
              }
          ...
          throw new McpError(
            ...
            `Failed to read EPUB resource: ${error}`
          );
        }
      }
    );
   ...
   * 参数验证方法 - 简化版本
   */
  private validateEpubInfoArgs(args: any): EpubInfoArgs {
  ...
  private validateEpubTocArgs(args: any): EpubTocArgs {
    if (typeof args.filePath !== 'string') {
      throw new McpError(ErrorCode.InvalidParams, 'filePath must be a string');
    }
    return { filePath: args.filePath };
  ...
  private validateEpubExtractTextArgs(args: any): EpubExtractTextArgs {
    if (typeof args.filePath !== 'string') {
      throw new McpError(ErrorCode.InvalidParams, 'filePath must be a string');
    }
    const result: EpubExtractTextArgs = { filePath: args.filePath };
    if (args.chapterIds && Array.isArray(args.chapterIds)) result.chapterIds = args.chapterIds;
    if (args.maxLength && typeof args.maxLength === 'number') result.maxLength = args.maxLength;
    return result;
  }

  private validateEpubSearchArgs(args: any): EpubSearchArgs {
    if (typeof args.filePath !== 'string') {
      throw new McpError(ErrorCode.InvalidParams, 'filePath must be a string');
    }
    ...
    const result: EpubSearchArgs = { filePath: args.filePath, query: args.query };
    if (typeof args.caseSensitive === 'boolean') result.caseSensitive = args.caseSensitive;
    return result;
  }
   ...
   * 处理EPUB信息工具
   */
  private async handleEpubInfo(args: EpubInfoArgs) {
    try {
   ...
   * 处理EPUB目录工具
   */
  private async handleEpubToc(args: EpubTocArgs) {
    try {
      const parser = new EpubParser(args.filePath);
      const info = await parser.getInfo();

      // 格式化目录显示
      const formatToc = (items: any[], level: number = 0): string => {
        let result = '';
   ...
   * 处理EPUB文本提取工具
   */
  private async handleEpubExtractText(args: EpubExtractTextArgs) {
    try {
      const parser = new EpubParser(args.filePath);
      const info = await parser.getInfo();
      ...
      if (args.chapterIds && args.chapterIds.length > 0) {
        chaptersToExtract = info.chapters.filter(ch => 
          args.chapterIds!.includes(ch.id)
        );
      }
   ...
   * 处理EPUB搜索工具
   */
  private async handleEpubSearch(args: EpubSearchArgs) {
    try {
      const parser = new EpubParser(args.filePath);
      ...
      const resultText = results.map((result, index) => 
        `${index + 1}. 章节: ${result.chapterTitle}\n   上下文: ...${result.context}...\n   位置: ${result.position}\n`
      ).join('\n');
